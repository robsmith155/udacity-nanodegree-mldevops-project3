...
def test_clean_data() -> None:
    ...
    try:
        ...
        logging.error(
            f"Testing clean_data: Expected no null values but found {df_clean.isnull().sum().sum()}"
        )
        raise err
    ...
    try:
        ...
        logging.info(
            f"Testing clean data: The cleaned dataset has {len(df_clean)} rows which is within the expected range."
        )
    except AssertionError as err:
        logging.error(
            f"Testing clean_data: After cleaning the dataset has {len(df_clean)} which is outside the expected range."
        )
        raise err
    ...
    try:
        ...
        logging.info(
            f"Testing clean_data: The cleaned dataset has the correct number of columns ({len(df_clean.columns)})"
        )
    except AssertionError as err:
        logging.error(
            f"Testing clean_data: The cleaned dataset has {len(df_clean.columns)} but expected 15."
        )
        raise err
...
def test_process_data_training(df_clean) -> None:
    ...
    try:
        ...
        logging.error(
            f"Testing process_data: Expected output data to be Numpy arrays but they are {type(X_train)} and {type(y_train)}"
        )
        raise err
    ...
    try:
        ...
        logging.error(
            "Testing process_data: Expected rank of y_train to be 1, but got {y_train.dim}."
        )
        raise err
...
def test_train_model(training_x, training_y) -> None:
    ...
    try:
        ...
        logging.error(
            f"Testing train_model: Expected the model to be of the type BalancedRandomForestClassifier but got {type(model)}."
        )
        raise err
